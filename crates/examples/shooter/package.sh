#!/usr/bin/env bash
# Package the shooter QUIC server into one archive you can scp to a server.
#
#   ./package.sh                 -> dist/shooter-server.tar.zst
#   ./package.sh --gzip          -> dist/shooter-server.tar.gz  (target has no zstd)
...
#
# On the target:
#   tar --zstd -xf shooter-server.tar.zst && cd shooter-server && ./run.sh
#
# The bundle carries only the devnet stack -- no surfpool (the shooter never ran an
...
# shooter server (devnet)

    tar --zstd -xf shooter-server.tar.zst   # or: tar -xzf shooter-server.tar.gz
    cd shooter-server
...
there -- built with the `shooter` feature, i.e. what `build_shooter.sh` produces.
