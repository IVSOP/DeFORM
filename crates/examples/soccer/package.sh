#!/usr/bin/env bash
# Package the soccer QUIC server into one archive you can scp to a server.
#
#   ./package.sh                 -> dist/soccer-server.tar.zst
#   ./package.sh --gzip          -> dist/soccer-server.tar.gz  (target has no zstd)
...
#
# On the target:
#   tar --zstd -xf soccer-server.tar.zst && cd soccer-server && ./run.sh
#
...
# soccer server (devnet)

    tar --zstd -xf soccer-server.tar.zst   # or: tar -xzf soccer-server.tar.gz
    cd soccer-server
...
there -- built with the `soccer` feature, i.e. what `build_soccer.sh` produces.
