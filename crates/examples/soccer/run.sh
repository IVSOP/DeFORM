#!/usr/bin/env bash
# Launch two soccer clients (each with its own wallet) plus the local docker stack.
#
...
cd "$CRATES_DIR"
cargo run --release -p soccer --no-default-features --features="client,server,anchor,foc-ping,metrics,20hz" -- run ${WALLET1:+--wallet="$WALLET1"} &
pids+=($!)
