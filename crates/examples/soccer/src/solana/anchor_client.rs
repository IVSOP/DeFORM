...
use crate::{
    ...
    soccer_logic::{SoccerError, SoccerGame, SoccerInputs},
};

pub const GAME_PROGRAM: Pubkey = crate::generated::ANCHOR_PROGRAM_ID;

#[derive(Clone)]
pub struct SoccerAnchorClient;

impl GameProgramClient<SoccerGame> for SoccerAnchorClient {
    ...
    fn create_lobby_ix(
        ...
        id: u64,
        network: Network,
    ) -> Result<Instruction, SoccerError> {
        Ok(CreateLobby {
            user,
            lobby,
    ...
    fn ready_ix(&self, args: ReadyArgs) -> Result<Instruction, SoccerError> {
        match args {
    ...
    fn write_and_close_ix(
        ...
        let game_state = match &lobby.state {
            LobbyState::NotStarted(_) => Err(SoccerError::LobbyNotStarted)?,
    ...
    fn force_close_ix(&self, admin: Pubkey, account: Pubkey) -> Result<Instruction, SoccerError> {
    ...
    fn start_ix(
        ...
        for user in not_started.player_status.keys() {
            let (inputs_account, _) =
                InputsAccount::<SoccerGame>::find_program_address(lobby_metadata.id, user, &game);
    ...
    fn set_inputs_ix(
        ...
        lobby_id: u64,
        inputs: &HashMap<u64, SoccerInputs>,
    ) -> Result<Instruction, SoccerError> {
        let batch_inputs_bytes =
            wincode::serialize(inputs).map_err(|e| SoccerError::SerializeInputs(e.to_string()))?;
    ...
    fn init_crank_ix(
        ...
        .try_to_vec()
        .map_err(|e| SoccerError::ScheduleCrank(e.to_string()))?;
