...
cd "$CRATES_DIR"
# cargo run --locked --release -p pong -- run ${WALLET1:+--wallet="$WALLET1"} &
cargo run --locked --release -p pong --no-default-features --features="client,server,anchor,foc,metrics,20hz" -- run ${WALLET1:+--wallet="$WALLET1"} &
pids+=($!)
cargo run --locked --release -p pong -- run ${WALLET2:+--wallet="$WALLET2"} &
pids+=($!)
