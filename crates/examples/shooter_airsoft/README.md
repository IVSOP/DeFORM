...
The map is a 24 × 36 m plywood training house inside a warehouse: rooms and doorways, firing windows, staggered central cover, two flank routes, crates, a raised deck with stairs and rails, roof trusses, skylights, and fluorescent fixtures. Players alternate between the two Blender spawn markers in sorted player-key order; additional players spread along each spawn line. The bot follows a Vleue Navigator navmesh built once from the exported collision geometry on the ground plane. Obstacle bounds and arena edges reserve the capsule radius plus 6 cm of clearance; synchronous path queries keep navigation available to both headless and client bots without asynchronous state. Movement follows navmesh waypoints even while aiming at a visible opponent, so seeing over cover does not bypass navigation. Unreachable routes stop movement. Navigation remains ground-only: bots do not plan jumps or routes across the raised deck. It fires at most once every 0.5 s, aiming at a random point in a disk around the opponent's torso. The disk radius is 8% of distance (about a 4.6° cone): unobstructed shots within 3 m fit inside the capsule, while distant shots can miss. Randomness is derived from shot sequence, round, and bot identity so replay stays deterministic. These rules apply to both the offline opponent and the menu's bot toggle; human weapon cooldown remains 0.2 s.
...
Body hits also create entry and exit wounds, a forward blood spray, and a smaller spray back from the entrance. Wound meshes conform to the capsule and remain attached as the corpse moves and tumbles. Droplets are Avian dynamic sphere bodies that inherit body velocity and fall under gravity at 60 Hz. Static cuboid colliders match the exported map; Avian contact points and normals place an alpha-blended blood decal on the first collision, then the droplet despawns. Droplets collide with the map only, not each other. This uses discrete collision detection, with no raycasts or swept CCD; a droplet that misses a thin wall simply continues until another collision or expiry. Further hits on a corpse add wounds and spray but never award another point or score hit marker. Multiple wounds are cosmetic: the first hit still ends the round, rather than adding a health/damage system.

Blood rendering is bounded to 96 live droplets, 128 surface stains, and 32 wound marks per player (sixteen entry/exit pairs); older marks are retired at those limits. Surface splatters are 1.5× their original width and height (0.15–0.39 m decal quads). Droplets expire after 1.5 seconds. Blood is cleared on round reset and when leaving a match. The original splatter texture is generated once in Rust, with a shared low-poly droplet mesh and no new shadow-casting lights. Blood physics runs locally in the rendering app, separate from the authoritative gameplay/rollback physics world. The authoritative hit event carries the victim ID and body-local entry/exit locations; clients handle particle trajectories and decals as cosmetic effects. As with other shot effects, already-presented blood is not retracted on a prediction correction, and late arrivals do not reconstruct expired shot events.
...
The camera defaults to **4× MSAA**, with no TAA or SSAO. The **Graphics** section in both the lobby menu and in-game scoreboard has live checkboxes for MSAA (4×/off), bloom, and VSync. All three default to enabled and remain selected across matches for the current app session. Disabling VSync requests `AutoNoVsync`; the desktop compositor can still pace presentation. Depth prepass remains enabled for impact and blood decals regardless of these settings. Baked occlusion replaces SSAO on the static scenery. Native rendering supports MSAA with these decals; Bevy's WebGPU forward decals require MSAA off if a browser client is added later.

Map textures (including embedded glTF textures), the lightmap, and procedural decal textures receive complete mip chains once at load time, with trilinear filtering and 4× anisotropic filtering. Color mips are averaged in linear light and transparent decal colors are alpha-weighted to avoid dark fringes. Linear normal/roughness/metalness maps retain linear filtering; texture wrap modes are preserved. No Blender re-export is required.
...
All example launchers and Docker builds use `nightly-2026-04-03` and enforce
`Cargo.lock`. Docker base images and Debian package snapshots are pinned, and
server bundles record their exact image ID. See [build versions](../../BUILD_VERSIONS.md)
for the pins and update procedure. This compiler has been checked with Bevy 0.19.1.
...
This optional probe runs a stationary, bot-disabled scene in a requested 1920×1080 window (the logged physical size is authoritative if the compositor overrides it), disables background-window throttling, requests `AutoNoVsync` presentation, and measures the current setup, the same setup with cosmetic blood physics paused, and all eight lamps active with 2048² shadow maps. All stages use the same bake, two sun cascades, and 4× MSAA; this isolates shadow-budget costs, not the entire pre-bake rendering configuration. Each stage warms up for four seconds, measures six seconds, and logs FPS, frame times, physical framebuffer resolution and GPU pass timings before exiting. The compositor may still pace presentation, so measured FPS is not necessarily the GPU throughput ceiling. Normal play does not enable these diagnostics. `run.sh` launches two clients, which share the GPU; its performance is not directly comparable with a single-client probe.
