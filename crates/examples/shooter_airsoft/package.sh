#!/usr/bin/env bash
# Package the shooter_airsoft QUIC server into one archive you can scp to a server.
#
#   ./package.sh                 -> dist/shooter_airsoft-server.tar.zst
#   ./package.sh --gzip          -> dist/shooter_airsoft-server.tar.gz  (target has no zstd)
#   ./package.sh --no-keypair    -> bundle without admin.json (copy it over yourself)
#   PLATFORM=linux/arm64 ./package.sh
#   CARGO_BUILD_FEATURES="server,anchor" ./package.sh
#
# On the target:
#   tar --zstd -xf shooter_airsoft-server.tar.zst && cd shooter_airsoft-server && ./run.sh
#
# The bundle carries only the devnet stack -- no surfpool (the shooter_airsoft never ran an
# ephemeral validator anyway; it is Web2-only).
...
# shooter_airsoft server (devnet)

    tar --zstd -xf shooter_airsoft-server.tar.zst   # or: tar -xzf shooter_airsoft-server.tar.gz
    cd shooter_airsoft-server
...
there -- built with the `shooter_airsoft` feature, i.e. what `build_airsoft_shooter.sh` produces.
Clients then connect to `<host>:<PORT>` with the network set to Devnet.
