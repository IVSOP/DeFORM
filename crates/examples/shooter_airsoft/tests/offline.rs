...
fn offline_backend_runs_the_sim() {
    ...
    while std::time::Instant::now() < deadline {
        ...
        if let LobbyState::Ongoing(ongoing) = &state.lobby.state {
            ...
            saw_shot |= !ongoing.tick_info.game_state.shots.is_empty();
            assert!(
    ...
    assert!(saw_shot, "held fire should have spawned shots");
...
fn visual_state_does_not_jump_backwards() {
    ...
    // Strafe across the clear spawn zone (yaw 0 => +X).
    let inputs = ShooterInputs {
        move_x: 100,
        ..Default::default()
    };
    ...
    while start.elapsed() < Duration::from_millis(1_000) {
        client.set_inputs(inputs.clone()).expect("send inputs");
        {
            let state = client.read_state().expect("read state");
            if let LobbyState::Ongoing(ongoing) = &state.lobby.state
                && let Some(ps) = ongoing.tick_info.game_state.players.get(&me)
            {
                samples.push(-ps.pos.x);
            }
        }
